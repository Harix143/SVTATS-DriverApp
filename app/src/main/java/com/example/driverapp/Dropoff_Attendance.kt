...
class Dropoff_Attendance : AppCompatActivity() {

    var Phone_No: String? = null
    var recyclerView: RecyclerView? = null
    var adapter: DropoffAttendanceAdapter? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_dropoff_attendance)

        val sessionManager =
            DriverSessionManager(
                this@Dropoff_Attendance,
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(snapshot.exists()) {
                    for (childDS in snapshot.children) {
                        ...
                        holder.add(cm)
                        adapter = DropoffAttendanceAdapter(holder, applicationContext)
                        recyclerView!!.setAdapter(adapter)
                    }
                }
                else{
                    Toast.makeText(this@Dropoff_Attendance, "No Student Assigned Yet!", Toast.LENGTH_SHORT).show();
                    finish()
                }
            }
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Dropoff_Attendance, error.message, Toast.LENGTH_SHORT).show();
            }
        })
